...
def merge_data(kk): #alku,loppu):
	...
	con.close()
	df['validdate'] = pd.to_datetime(df['validdate'], unit='s') 
	...
	#print(df['validdate'])
	df['fcdate'] = pd.to_datetime(df['fcdate'], unit='s')

	# define times for obs retrieval
	alku = df['validdate'].min()	
	loppu = df['validdate']. max()
	...
	df_new = df_new.dropna( how='all', subset=['RH_PT1M_AVG','TA_PT1M_AVG','WS_PT10M_AVG','WG_PT10M_MAX'])
...
print("jesh")
#for looppi kaikkien vuoden 2022 sqite taulujen yli
...
# modaa, niin ettö voi hakea vuoden kerrallaan
